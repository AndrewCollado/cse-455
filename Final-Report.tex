...
% FIX: Pass 'numbers' option to natbib BEFORE nips_2017 loads it to prevent option clash
...
% Adjust float separation to prevent white gaps on pages with figures
...
Accurate financial forecasting tools benefit society in three key ways: (1) \textbf{Retirement Security}: They enable better long-term planning for individual investors and pension funds. (2) \textbf{Systemic Stability}: Improved tail-risk modeling helps institutions anticipate crashes, potentially reducing the severity of future financial crises. (3) \textbf{Democratization}: Open-source implementations like ours democratize access to sophisticated risk management tools previously available only to large hedge funds.

\section{Related Works}\label{sec:past}

\textbf{Generative Models in Finance.}
Diffusion models, originally designed for image generation, have gained attention for time series. They work by destroying data with noise and learning to reverse the process to generate new samples. \citet{Meijer2024} survey diffusion for forecasting, while \citet{Briazkalo2025} apply them to financial series. Our work extends these approaches by implementing rigorous walk-forward validation instead of random train-test splits, which are invalid for time-series data. In contrast to \citet{Kim2025} who incorporate Geometric Brownian Motion structure, or \citet{Park2025} who apply diffusion to price chart images, we focus on raw return forecasting. Similarly, unlike \citet{Dogariu2022} who use GANs for synthetic data generation, our work addresses conditional probabilistic forecasting with quantitative evaluation.
...
\end{itemize}
We adhere to the principles of \citet{Bergmeir2012} and \citet{Borra2010}, who emphasize that standard cross-validation causes leakage in time series; thus, we implement a strict walk-forward approach.
